...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Util.Encoder
{
    public class EncoderHelper
    {
        ...
        protected static List<ProfileValues> GetProfilesFromBasicMatches(String encoderName, ContentData content, Boolean trailer)
        {
            ...
            log.Debug("No of PlayTypes= " + assetFormatTypes.Count.ToString());
            log.Debug("displayAspectRating= " + displayAspectRating);
            log.Debug("CRO= " + contentRightsOwner);
            log.Debug("SubtitleFormat= " + subtitleFormat);
            log.Debug("No of subtitles= " + subtitleLanguages.Count);
            ...
            log.Info("Found " + profileMatches.Count() + " profiles from first search criterias");
        ...
        protected static List<ProfileValues> LoadAndParsePresetList()
        {
            ...
            {
                log.Debug("Error when closing connection to DB", exc);
            }
            return ret;
        }
        ...
        private static ProfileValues BuildProfile(string profileID, OleDbConnection connection)
        {
            ...
            try
            {
               ...
               // ret.SubtitleType = GetValue(reader, "SUBTITLES_TYPE");
                ...
                //ret.AudioTrackPids = new List<string>();
                //value = GetValue(reader, "AUDIO_PIDS");
                //if (!String.IsNullOrEmpty(value))
                //    ret.AudioTrackPids.Add(value);
