...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Util.XmlFunctionality.Plugins
{
    public class CableLabsXmlTranslator
    {
        ...
        public ContentData TranslateXmlToContentData(IngestConfig ingestConfig, XmlDocument contentXml)
        {
            ...
            //if (ContentPropertyConfig == null)
            //    throw new Exception("ContentProperty is not configured in the ConaxWorkflowManagerConfig.xml.");
            ...
            if (!String.IsNullOrEmpty(contentXml.BaseURI))
            {
                ...
                if (tmpurls.Length > 0)
                    content.Properties.Add(new Property("IngestXMLFileName", Path.Combine(tmpurls[tmpurls.Length - 3], tmpurls[tmpurls.Length - 2], tmpurls[tmpurls.Length - 1])));
            }

            //PublishInfo publishInfo = new PublishInfo();
            //publishInfo.DeliveryMethod = DeliveryMethod.Stream;
            //publishInfo.PublishState = PublishState.NeedsQA;
            //publishInfo.Region = ingestConfig.Region;
        ...
        private void ParseVideoAssetXML(IngestConfig ingestConfig, XmlElement MANode, XmlNodeList deviceTypeNodes, ContentData content, Boolean IsTraielr)
        {
            ...
            //XmlElement languagesNode = (XmlElement)MANode.SelectSingleNode("Metadata/App_Data[@Name='Languages']");
            //if (languagesNode != null)
            //    defaultAsset.LanguageISO = GetThreeLetterISOLanguageName(languagesNode.GetAttribute("Value")).ToUpper();
            ...
            //log.Debug("Didnt find a Resolution_Type looking for Resolution");
            //XmlElement resolutionNode = (XmlElement)MANode.SelectSingleNode("Metadata/App_Data[@Name='Resolution']");
            //if (resolutionNode != null && !String.IsNullOrEmpty(resolutionNode.GetAttribute("Value")))
            //{
            //     log.Debug("Setting resolution= " + resolutionNode.GetAttribute("Value"));
            //     ConaxIntegrationHelper.SetResolution(defaultAsset, resolutionNode.GetAttribute("Value"));
            //}
            ...
            //if (Boolean.Parse(systemConfig.GetConfigParam("AddExcludedStreamAssetOnXMLIngest")))
            //{
            //    // Add dummy asset for remaining stream types
            //    foreach (AssetFormatType aft in Enum.GetValues(typeof(AssetFormatType)).Cast<AssetFormatType>())
            //    {
            //        if (!assets.Keys.Contains(aft))
            //        {
            ...
            //            asset.FileSize = defaultAsset.FileSize;
            //            asset.Properties.AddRange(defaultAsset.Properties.ToArray());
            //            assets.Add(aft, asset);
            //            ConaxIntegrationHelper.AddAssetFormatTypeToAsset(asset, aft);
            //        }
            //    }
            //}
