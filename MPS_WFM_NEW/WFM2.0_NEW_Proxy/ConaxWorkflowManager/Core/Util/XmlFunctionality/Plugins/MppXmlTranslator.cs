...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Util.XmlFunctionality.Plugins
{
    public class MppXmlTranslator
    {
        ...
        public XmlDocument TranslateContentDataToXml(ContentData contentData)
        {
            ...
            xmlStr += "<Identification name=\"" + SecurityElement.Escape(contentData.Name) + "\" externalId=\"" + contentData.ExternalID + "\"/>";
            ...
            if (contentData.Assets.Count > 0)
            {
                ...
                foreach (Asset asset in contentData.Assets)
                {
                    ...
                    xmlStr += "<VideoAsset name=\"" + SecurityElement.Escape(asset.Name) + "\" " +
                ...
                if (videoAssetNodes != null)
                {
                    foreach (XmlElement videoAssetNode in videoAssetNodes)
                    {
                        ...
                        if (videoAssetNode.HasAttribute("objectId"))
                        {
                            ...
                            //log.Debug("Asset.ObjectID = " + asset.ObjectID.ToString());
                        }
                        //else
                        //{
                        //    //log.Debug("Asset have no objecId");
                        //    XmlElement MPPAssetContextNode = (XmlElement)videoAssetNode.SelectSingleNode("MPPAssetContext");
                        //    if (MPPAssetContextNode.HasAttribute("objectId"))
                        //    {
                        //        //log.Debug("MppAssetContext have objectId, using that");
                        //        //if (asset.ObjectID.HasValue)
                        //        //    log.Debug("ObjectID was " + asset.ObjectID.Value.ToString());
                        //        asset.ObjectID = UInt64.Parse(MPPAssetContextNode.GetAttribute("objectId"));
                        //        //log.Debug("Now ObjectID is " + asset.ObjectID.Value.ToString());
                        //    }
                        //}
                        //XmlElement MMPACTXNode = (XmlElement)videoAssetNode.SelectSingleNode("MPPAssetContext");
                        //if (MMPACTXNode != null && !String.IsNullOrEmpty(MMPACTXNode.GetAttribute("contentAssetServerName")))
                        //    asset.contentAssetServerName = XmlUtil.UnescapeXML(MMPACTXNode.GetAttribute("contentAssetServerName"));
