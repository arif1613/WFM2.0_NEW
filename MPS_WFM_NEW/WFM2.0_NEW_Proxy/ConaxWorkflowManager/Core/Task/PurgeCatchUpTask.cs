...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Task
{
    public class PurgeCatchUpTask : BaseTask
    {
        ...
        private void DeleteOldCatchupContent(EPGChannel epgChannel, DateTime deleteTime)
        {
            ...
            log.Debug("contents to delete " + contents.Count);

            var syncedContents = contents.Where(c =>c.Properties.Exists(p => p.Type.Equals(CatchupContentProperties.EpgIsSynked) && p.Value.Equals(bool.TrueString))).ToList();

            foreach (ContentData content in syncedContents)
            {
