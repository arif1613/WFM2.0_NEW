...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Task
{
    public class PurgeNPVRTask : BaseTask
    {
        ...
        private Dictionary<ulong, List<EpgContentInfo>> listOfOldContentsThatAreMissedFromPurge = new Dictionary<ulong, List<EpgContentInfo>>();
        private double _numberOfDaysForExpiredEPG;
        ...
        public override void DoExecute()
        {
            ...
            _numberOfDaysForExpiredEPG = Double.Parse(managerConfig.GetConfigParam("DaysForExpiredEPG"));
            ...
            try
            {
                ...
                GetEPGsForAllChannelsMissedFromPurgeNPVR(_numberOfDaysForExpiredEPG, executeTime);
                SetSortedEPGsReadyToNPVRPurgePropertyToTrue();
            }
            catch (Exception ex)
            {
        ...
        private void GetEPGsForAllChannelsMissedFromPurgeNPVR(double numberOfDaysForExpiredEpg, DateTime executeTime)
        {

            List<EPGChannel> epgChannels = CatchupHelper.GetAllEPGChannels();

            //getting all contents from mpp and check that there are no recordings on them, add them to list and change the property ReadyToNpvrPurge 
            foreach (EPGChannel epgChannel in epgChannels)
            {
                List<EpgContentInfo> epgs = mppWrapper.GetEpgContentInfoByChannel2(epgChannel, numberOfDaysForExpiredEpg, executeTime);
                if (!listOfOldContentsThatAreMissedFromPurge.ContainsKey(epgChannel.MppContentId))
                {
                    listOfOldContentsThatAreMissedFromPurge.Add(epgChannel.MppContentId, epgs);
                }

            }
        }

        private void SetSortedEPGsReadyToNPVRPurgePropertyToTrue()
        {
            //Preparation of deletion by updating the property for contents which are old and missed by getting purged.
            if (listOfOldContentsThatAreMissedFromPurge.Count > 0)
            {
                foreach (KeyValuePair<ulong, List<EpgContentInfo>> kvp in listOfOldContentsThatAreMissedFromPurge)
                {
                    List<EpgContentInfo> epgValues = kvp.Value;
                    foreach (EpgContentInfo contentInfos in epgValues)
                    {

                        if (contentInfos.Content == null)
                        {
                            continue;
                        }

                        if (!contentInfos.Content.Properties.Where(p => p.Type == "ReadyToNPVRPurge").Any())
                        {
                            //checking if service has no recordings and content is older than defined number of days
                            if (ConaxIntegrationHelper.ServiceHasRecordingPropertyStateCount(contentInfos.Content, false) > 0)
                            {
                                if (ConaxIntegrationHelper.HasNoFailedAttempts(contentInfos.Content))
                                {
                                    log.Debug(contentInfos.Content.Name + "- external ID: " + contentInfos.Content.ExternalID + "- MPP content ID: " + contentInfos.Content.ID + " is found older than a configured number of days, adding SetReadyToNPVRPurgeProperty property with TRUE.");
                                    Property ReadyToNPVRPurge = ConaxIntegrationHelper.SetReadyToNPVRPurgeProperty(contentInfos.Content, true);

                                    mppWrapper.AddContentProperty(contentInfos.Content.ID.Value, ReadyToNPVRPurge);
                ...
                listOfOldContentsThatAreMissedFromPurge.Clear();
            }
        }
        ...
        private List<ContentData> GetNPVRPurgeReadyContents()
        {
            ...
            foreach (EPGChannel channel in epgChannels)
            {
                ...
                csp.Properties.Add(CatchupContentProperties.ChannelId, channel.MppContentId.ToString("G"));
                csp.Properties.Add(CatchupContentProperties.ReadyToNPVRPurge, true.ToString());
                csp.MaxReturn = managerConfig.FetchNumberOfReadyToPurgeEPG;
                csp.EventPeriodTo = epgHistoryDate;
                List<ContentData> contents = mppWrapper.GetContentFromProperties(csp, true);                
