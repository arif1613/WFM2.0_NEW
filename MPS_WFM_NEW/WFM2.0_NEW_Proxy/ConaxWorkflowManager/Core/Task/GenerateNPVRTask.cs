...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Task
{
    public class GenerateNPVRTask : BaseTask
    {
        ...
        public override void DoExecute()
        {
            ...
            List<System.Threading.Tasks.Task<TPLTaskResult>> TPLTasks = new List<System.Threading.Tasks.Task<TPLTaskResult>>();
            ...
            while (TPLTasks.Count > 0)
            {
                ...
                try
                {
                    ...
                    if (res is ArchiveAssetTPLTaskResult) {
                        ...
                        if (((ArchiveAssetTPLTaskResult) res).IsArchived)
                        {
                            ...
                            PrintLogToLog4NetWithThreadContextData("Content " + epg.Content.Name + " with id " + epg.Content.ID + ", externalId= " + epg.Content.ExternalID + " is Archived", epg.Content);
                        }
                    }
                }
                ...
                while ((ContentsInArchiving.Count < Config.GetConaxWorkflowManagerConfig().MAXArchiveThreads) &&
                        ContentsWaitingForProcess.Count > 0)
                {   // add new archvie asset task
                    ...
                    System.Threading.Tasks.Task<TPLTaskResult> archiveAssetTask = addArchiveAssetTask(epg);
        ...
        private void LogQeueSize() {
            ...
            if (ContentsWaitingForProcess.Count > 0 ||
                ContentsInArchiving.Count > 0 ||
                ContentsWaitingForUpdateRecordings.Count > 0 ||
                ContentsInUpdateRecordings.Count > 0) {


                    if (ContentsWaitingForProcess.Count > wfmConfig.WarningThresholdForNumberOfEPGInWaitList)
                        log.Error(ContentsWaitingForProcess.Count + " contents is waiting to be processed. Exceeding warning threshold something might not be in order.");
                    else 
                        log.Debug(ContentsWaitingForProcess.Count + " contents is waiting to be processed.");
                    log.Debug(ContentsInArchiving.Count + " contents is archving assets.");
                    log.Debug(ContentsWaitingForUpdateRecordings.Count + " contents is waiting to update NPVR_Recordings.");
                    log.Debug(ContentsInUpdateRecordings.Count + " contents is updating the NPVR_recordings.");
            }
        }
        ...
        private void PrintQueuesToExtraLogs()
        {
            ...
            PrintQueueToSeparateLogs(dateString, "ContentsWaitingForProcessList", ContentsWaitingForProcess);
            PrintQueueToSeparateLogs(dateString, "ContentsInArchivingList", ContentsInArchiving);
            PrintQueueToSeparateLogs(dateString, "ContentsWaitingForUpdateRecordingsList", ContentsWaitingForUpdateRecordings);
            PrintQueueToSeparateLogs(dateString, "ContentsInUpdateRecordingLists", ContentsInUpdateRecordings);
        }
        ...
        private System.Threading.Tasks.Task<TPLTaskResult> addFetchNewEPGWithRecordingTask(List<UInt64> inqueneNProcess)
        {
            System.Threading.Tasks.Task<TPLTaskResult> fetchNewEPGWithRecordingTask = System.Threading.Tasks.Task<TPLTaskResult>.Factory.StartNew(() =>
            {
                ...
                FilterOutEPGWithNoRecordingsUpdateInBulk(newEpgs);
        ...
        private void FilterOutEPGWithSuccessfulLastTry(List<EPG> newEPG) {
            log.Debug("Filtering out content not yet ready to start. " + newEPG.Count + " EPG contents are ready.");
            Int32 index = 0;
            var stopwatch = new Stopwatch();
            stopwatch.Start();
            while (index < newEPG.Count)
            {
                ...
                try
                {
                    HandleArchiveResult(content); // Updaterar content i MPP:n
                }
                catch (Exception ex)
                {
                    log.Warn("Failed to handle archiving results for Content " + content.Name + ", Id= " + content.ID + ", ExternalID= " + content.ExternalID + ". Will skip processing this content.", ex);
            ...
            while (index < newEPG.Count)
            {
                if (newEPG[index].Recordings.Count == 0)
                {
                    ...
                    if (MaxTriesReached(content))
                    {
                        ...
                        {
                            log.Warn("Failed to set content property NPVRRecordingsstState to 'NotArchived' in MPP, for Content " + content.Name + ", Id=" + content.ID.Value + ", ExternalID= " + content.ExternalID, ex);
                        }
                    }
        ...
        private void FilterOutEPGWithNoRecordingsUpdateInBulk(List<EPG> newEPG)
        {
            ...
            while (index < newEPG.Count)
            {
                if (newEPG[index].Recordings.Count == 0)
                {
                    ...
                    if (MaxTriesReached(content))
                    {
                        List<KeyValuePair<String, Property>> prToUpdate = new List<KeyValuePair<String, Property>>();
                        List<Property> unKnownProeprties = ConaxIntegrationHelper.GetAllNPVRAssetArchiveStateByState(content,
                            NPVRAssetArchiveState.Unknown);
                        foreach (Property unKnownProeprty in unKnownProeprties)
                        {
                            unKnownProeprty.Value = NPVRAssetArchiveState.NotArchived.ToString();
                            prToUpdate.Add(new KeyValuePair<string, Property>("UPDATE", unKnownProeprty));
                        }

                        UpdatePropertiesForContentParameter updateParameter = new UpdatePropertiesForContentParameter();
                        updateParameter.Content = content;
                        updateParameter.Properties = prToUpdate;
                        updates.Add(updateParameter);
                     }
                    PrintLogToLog4NetWithThreadContextData("No new recordings found for content " + content.ID + " " +
                                content.ExternalID, content);
            ...
            if (updates.Count > 0)
            {
                ...
                {
                    mppWrapper.UpdateContentsPropertiesInChunks(updates);
                    log.Debug(updates.Count + "number of content(s) are updated in bulk for for None Archived Tenants, the content(s) property NPVRAssetArchiveState value changed  from \"Unknown\" to \"NotArchived\"");
                }
                catch (Exception ex)
                {

                    log.Warn("Failed to update in bulk.", ex);
                }
            }
            ...
            foreach (EPG epg in newEPG)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (epgChannel == null)
                        {
                            log.Warn("Could not get EPGChannel for content. " + epg.Content.Name + " " + epg.Content.ID + " " + epg.Content.ExternalID + ". Skipping this content.");
                            continue;
                        }
                    ...
                    {
                        log.Warn("Could not get EPGChannel for content. " + epg.Content.Name + " " + epg.Content.ID + " " + epg.Content.ExternalID + ". Skipping this content.", ex);
                        continue;
                    }
                }
                ...
                {
                    if (!epgChannel.ServiceEPGConfigs.ContainsKey(service.ObjectID.Value))
                    {
                        log.Warn("The EPG Content with MppContentId " + epg.Content.ID + " has the service with objectId " + service.ObjectID.Value + " in it's Content Agreement, but no configuration can be found for service and channel (channel MPP content id: " + epgChannel.MppContentId + ").");
                        continue;
                    }    
        ...
        private Boolean ArchiveNPVRAssets(EPG epg) {
            ...
            //PrintLogToLog4NetWithThreadContextData("minStart: " + minStart.ToString("yyyy-MM-dd HH:mm:ss") + " maxEnd:" + maxEnd.ToString("yyyy-MM-dd HH:mm:ss"), content);
        ...
        private void UpdateNPVRRecordingsInCubi(NPVRRecordingUpdateList recordingsToUpdateList)
        {
            ...
            Parallel.ForEach(recordingsToUpdateList.Recordings, kvp =>
            {
                ...
                foreach (KeyValuePair<ContentData, List<NPVRRecording>> contentRecordings in recordingPerContent)
                {
                    ...
                    String serviceViewIso;

                    try
                    {
                        serviceViewIso = epgChannel.ServiceEPGConfigs[serviceObjectId].ServiceViewLanugageISO;
                    }
                    catch (Exception ex)
                    {
                        log.Warn("EPG content (" + content.Name + ", Id: " + content.ID.Value + ", External Id_ " + content.ExternalID + ") have recordings in service " + serviceObjectId + ", but no configuration can be found for service and channel (channel MPP content id: " + epgChannel.MppContentId + ").");
                        continue;
                    }
            ...
            if (MaxTriesReached(content))
            {
                ...
                if (ConaxIntegrationHelper.HasNoFailedAttempts(content))
                {
                    ...
                    if (allNPVRAssets.Count == notArchivedAssets.Count)
                    {
                        ...
                        {
                            log.Warn("Failed to update content property NPVRRecordingsstState in MPP, for Content " + content.Name + ", Id= " + content.ID + ", ExternalID= " + content.ExternalID, ex);
                            throw;
                        }
                        ...
                        {
                            log.Warn("Failed to add content property ReadyToNPVRPurge in MPP, to Content " + content.Name + ", Id= " + content.ID + ", ExternalID= " + content.ExternalID, ex);
                            throw;
                        }
                    ...
                    else
                    {
                        ...
                        {
                            log.Warn("Failed to update archived content in in MPP. Contnent name: " + content.Name + ", Id= " + content.ID + ", ExternalID= " + content.ExternalID, ex);
                            throw;
                        }
        ...
        private void UpdateLastAttemptState(ContentData content, UInt64 serviceObjectId, LastAttemptState state)
        {
            ...
            {
                log.Warn("Failed to add or update property LastAttemptStateInService for content " + content.Name + ", Id= " + content.ID + ", ExternalID= " + content.ExternalID, ex);
            }       
        }
        ...
        private void ArchiveAssets(ContentData content, List<UInt64> serviceobjectId, DateTime startTime, DateTime endTime)
        {
            ...
            BaseEncoderCatchupHandler hlshHandler = managerConfig.HLSCatchUpHandler;
            if (hlshHandler is SeaWellHLSCatchupHandler ||
                hlshHandler is NullHLSCatchupHandler)
            {
        ...
        private List<ContentData> GetAllUnprocessedMPVRContents(List<UInt64> contentToIgnore, Int32 retryIntervall)
        {
            ...
            stopwatch.Stop();
            log.Debug("Done GetAllEPGChannels. Took " + stopwatch.ElapsedMilliseconds + " ms");
            ...
            MppCallTimes mppCallTimes = new MppCallTimes();

            XmlDocument contentToIgnoreXml = CreateContentToIgnoreXml(contentToIgnore);
            ...
            foreach (EPGChannel channel in channels)
            {
                ...
                try
                {
                    var contents = GetUnprocessedNPVRContents(UTCMaxEndtime, channel, contentToIgnoreXml, retryIntervall, mppCallTimes);
                    allcontents.AddRange(contents);
                }
                catch (Exception ex)
                {
                    log.Warn(
                        "Failed to get EPGs from MPP for channel with MPP ContentId " + channel.MppContentId, ex);
                }
            }
            stopwatch.Stop();
            log.Debug("Done loading contents from all channels. Took " + stopwatch.ElapsedMilliseconds + " ms");
            var mppCallsTotalTime = mppCallTimes.GetAllServicesForContentTime.Sum() + mppCallTimes.GetOngoingEPGsTime.Sum();
            log.Debug("Calls to MPP when loading contents from all channels took " + mppCallsTotalTime + " ms. (GetAllServicesForContent: " + mppCallTimes.GetAllServicesForContentTime.Sum() + " ms, GetOngoingEPGs: " + mppCallTimes.GetOngoingEPGsTime.Sum() + " ms)");

            return allcontents;
        }

        private XmlDocument CreateContentToIgnoreXml(List<ulong> contentToIgnore)
        {
        ...
        private List<ContentData> GetUnprocessedNPVRContents(DateTime UTCMaxEndtime, EPGChannel channel, XmlDocument contentToIgnore, Int32 retryIntervall, MppCallTimes mppCallTimes)
        {
            ...
            contents = mppWrapper.GetOngoingEpgs(contentToIgnore, channel.MppContentId, UTCMaxEndtime, retryIntervall);
            
            stopwatch.Stop();
            mppCallTimes.GetOngoingEPGsTime.Add(stopwatch.ElapsedMilliseconds);
            ...
            stopwatch.Stop();
            mppCallTimes.GetAllServicesForContentTime.Add(stopwatch.ElapsedMilliseconds);
