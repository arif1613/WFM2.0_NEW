...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Communication
{
    ...
    public class MPPIntegrationServicesWrapper
    {
        ...
        public List<EpgContentInfo> GetEpgContentInfoByChannel(UInt64 channelId, Int32 epgHistoryInHours, DateTime executeTime)
        {
            ...
            try
            {
               // List<EPGChannel> channels = CatchupHelper.GetAllEPGChannels();
                ...
                EPGChannel epgChannel = CatchupHelper.GetEPGChannel(channelId, new MppCallTimes()); //channels.FirstOrDefault(c => c.MppContentId == channelId);
        ...
        public List<EpgContentInfo> GetEpgContentInfoByChannel2(EPGChannel epgChannel, double numberOfDaysForExpiredEpg, DateTime executeTime)
        {
            try
            {
                DateTime eventPeriodTo = executeTime.AddHours(-(numberOfDaysForExpiredEpg * 24));


                ContentSearchParameters param = new ContentSearchParameters();
                param.ContentRightsOwner = epgChannel.ContentRightOwner;
                param.EventPeriodTo = eventPeriodTo;

                param.Properties.Add(SearchProperty.S_ChannelIdContentType, epgChannel.MppContentId + ":" + ContentType.CatchupTV.ToString("G"));
                param.ZipReply = _zipReply;
                return GetEpgContents2(param, epgChannel);
            }
            catch (Exception ex)
            {
                log.Error("Failed to get EPG from MPP for channel " + epgChannel.MppContentId, ex);
                return null;
            }
        }

        private List<EpgContentInfo> GetEpgContents2(ContentSearchParameters contentSearchParameters, EPGChannel epgChannel)
        {
            var epgContentInfoList = new List<EpgContentInfo>();
            var syncedEpgContents = new EpgContentInfo();
            var nonSyncedEpgContents = new EpgContentInfo();

            try
            {
                List<ContentData> contents = GetContentFromProperties(contentSearchParameters, true);
                var objectIdsOfServicesPublishedTo = epgChannel.ObjectIdsOfServicesPublishedTo;
                foreach (ContentData content in contents)
                {
                    EpgContentInfo epgInfo = new EpgContentInfo();

                    //epgInfo.Content = RemoveAssets(content);//Why are we removing Assets here,that is why asset count is 0 when fetching from MPP?? :S
                    ...
                    epgInfo.ChannelPublishedToNewService = false;
                    var addedProperties = AddEpgSyncedOnPropertiesIfMissing(epgInfo, objectIdsOfServicesPublishedTo);
                    if (addedProperties)
                        epgInfo.ChannelPublishedToNewService = true;


                    epgInfo.IsSynced = EpgIsSynced(epgInfo);

                    if (epgInfo.IsSynced)
                        epgContentInfoList.Add(epgInfo);
                    else
                        epgContentInfoList.Add(epgInfo);
                }

                epgContentInfoList.Add(syncedEpgContents);
                epgContentInfoList.Add(nonSyncedEpgContents);
            }
            catch (Exception ex)
            {
                log.Error("Error fetching EPG Contents for Channel with MPP ContentId: " + epgChannel.MppContentId + " from MPP", ex);
                return null;
            }
            return epgContentInfoList;
        }

        private bool AddEpgSyncedOnPropertiesIfMissing(EpgContentInfo epgInfo, List<ulong?> objectIdsOfServicesPublishedTo)
        {
            bool propertiesAdded = false;

            foreach (var id in objectIdsOfServicesPublishedTo)
            {
                var epgSyncedOnProperty = epgInfo.Content.Properties.FirstOrDefault(
                    p => p.Type.Equals(CatchupContentProperties.EpgSyncedOn + ":" + id));

                if (epgSyncedOnProperty == null)
                {
                    epgInfo.Content.Properties.Add(new Property(CatchupContentProperties.EpgSyncedOn + ":" + id,
                                                                bool.FalseString));
        ...
        private bool EpgIsSynced(EpgContentInfo epgInfo)
        {
            var epgIsSynced = true;

            var falseEpgSyncedOnProperties =
                epgInfo.Content.Properties.FindAll(p => p.Type.StartsWith(CatchupContentProperties.EpgSyncedOn) && p.Value.Equals(bool.FalseString));

            return falseEpgSyncedOnProperties.Count == 0;
        }
