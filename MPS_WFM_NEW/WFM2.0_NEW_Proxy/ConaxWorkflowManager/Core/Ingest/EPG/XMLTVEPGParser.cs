...
namespace MPS.MPP.Auxiliary.ConaxWorkflowManager.Core.Ingest.EPG
{
    public class XMLTVEPGParser : IEPGParser
    {
        ...
        public Dictionary<UInt64, List<EpgContentInfo>> ParseEPGXML(XElement EPGXML, XElement EPGChannelConfigXML, XElement CatchUpFilterConfigXML, TimeZoneInfo feedtimeZone, List<EpgContentInfo> existingEpgs, List<EPGChannel> channels)
        {
            ...
            foreach (XElement programNode in EPGXML.Elements("programme"))
            {
                try
                {
                    ...
                    AddRatingsToContent(content, programNode);
                    AddCustomXmlToContent(content, programNode);
        ...
        private void AddCustomXmlToContent(ContentData content, XElement programNode)
        {
            XElement customXmlNode = programNode.Element("custom-xml");

            if (customXmlNode != null && customXmlNode.Nodes().Any())
                content.Properties.Add(new Property { Type = CatchupContentProperties.CustomXml, Value = string.Concat(customXmlNode.Nodes()).Trim() });
        }

        private void AddRatingsToContent(ContentData content, XElement programNode)
        {
