    ...
    public class CodeShopSmoothCatchupHandler : BaseEncoderCatchupHandler
    {
        ...
        public override void GenerateNPVR(ContentData content, UInt64 serviceObjId, String serviceViewLanugageISO, DeviceType deviceType, DateTime startTime, DateTime endTime)
        {
            ...
            if (recordResult.ReturnCode != 0)
            {
                ...
                if (IsFailedDependencyError(recordResult.Message))
                {
                    NPVRAssetLoger.WriteLog(content, "Asset may have been created in spite of the error (This can happen for 424 errors). Will try to delete the asset.");
                    log.Warn("Asset may have been created in spite of archiving error (This can happen for 424 errors). Will try to delete asset. Asset: " + asset.Name + ", Content " + content.Name + ", ExternalID: " + content.ExternalID);
                        // Will try to delete asset as assets it might have been created in spite 
                        var rc = wrapper.DeleteSmoothAsset(content, asset);
                        if(rc.ReturnCode == -1)
                            NPVRAssetLoger.WriteLog(content, "Failed to delete asset, due to: " + rc.Message);
        ...
        private bool IsFailedDependencyError(string errorMsg)
        {
            return (errorMsg.IndexOf("failed dependency", StringComparison.OrdinalIgnoreCase) >= 0 && errorMsg.Contains("424"));
        }
